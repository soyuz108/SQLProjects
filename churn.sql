...
SELECT MIN(subscription_start), MAX(subscription_start) FROM subscriptions Limit 100;
...
(SELECT month,
  ...
  SUM(is_active_87) AS sum_active_30,
  SUM(is_canceled_87) AS sum_canceled_87,
  SUM(is_canceled_30) AS sum_canceled_30
 FROM status
  GROUP BY month 
) SELECT month,
1.0 * sum_cancaled_87/sum_active_87 AS churn_rate_87,
1.0 * sum_cancaled_30/sum_active_30 AS churn_rate_30
